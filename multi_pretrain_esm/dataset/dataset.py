...
class pretrain_dataset(Dataset):
    def __init__(self, json_files, stru_alphabet, pdb_root, topk, num_rbf_node, num_rbf_edge, max_sl, device, seq_map_dict, mlm_probability=0.15):
        ...
        self.stru_alphabet = stru_alphabet
        ...
        self.device=device
        self.stru_map_dict = {'[PAD]': 0, '[UNK]': 1, '[CLS]': 2, '[SEP]': 3, '[MASK]': 4, 'L': 5, 'A': 6, 'G': 7, 'V': 8,
            'E': 9, 'S': 10, 'I': 11, 'K': 12, 'R': 13, 'D': 14, 'T': 15, 'P': 16, 'N': 17, 'Q': 18,
            'F': 19, 'Y': 20, 'M': 21, 'H': 22, 'C': 23, 'W': 24, 'X': 25}
        self.stru_map_dict_r = {v:k for k,v in self.stru_map_dict.items()}
        
        self.seq_map_dict = seq_map_dict
        self.seq_map_dict_r = {v:k for k,v in self.seq_map_dict.items()}
    ...
    def _pdb2dict(self, pdb_path, chain = 'A', paired = False, augment = (0,0)):
        if paired:
        ...
        if augment[1]:
            residues = residues[augment[0]: -augment[1]]
        else:
            residues = residues[augment[0]:]
    ...
    def tokenize_stru(self, seq, max_sl, padding=True, add_sep_cls = False):
        ...
        if type(seq) == tuple:
            ...
            if add_sep_cls:
                seq_id = [self.stru_map_dict['[CLS]']] + [self.stru_map_dict[x] for x in seq[0]] + [self.stru_map_dict['[SEP]']] + [self.stru_map_dict[x] for x in seq[1]] + [self.stru_map_dict['[SEP]']]
            else:
                seq_id = [self.stru_map_dict[x] for x in seq[0]] + [self.stru_map_dict[x] for x in seq[1]]
            ...
            # This changing doesn't affect seq_mask and seq_num which are used to extract structure feature
            if add_sep_cls:
                seq_id = [self.stru_map_dict['[CLS]']] + [self.stru_map_dict[x] for x in seq] + [self.stru_map_dict['[SEP]']]
            else:
                seq_id = [self.stru_map_dict[x] for x in seq]
        ...
        if padding:
            seq_id = np.pad(seq_id.astype(np.int32), (0, max_sl - len(seq_id)),
                            constant_values=self.stru_map_dict['[PAD]']) # save one place for cls
    ...
    def seq_id2token(self, ids):
        return ''.join([self.seq_map_dict_r[i] for i in ids])
    def stru_id2token(self, ids):
        return ''.join([self.stru_map_dict_r[i] for i in ids])

    def get_seq_mask(self, input_ids, input_ids_stru, seq_cdr=None):
        ...
        masked_indices[input_ids_stru == self.seq_map_dict['<cls>']] = False
        masked_indices[input_ids_stru == self.seq_map_dict['<pad>']] = False
        masked_indices[input_ids_stru == self.seq_map_dict['<eos>']] = False

        targets[~masked_indices] = -100 # We only compute loss on masked tokens            

        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(torch.full(input_ids.shape, 0.8)).bool() & masked_indices
        input_ids[indices_replaced] = self.seq_map_dict['<mask>']
    ...
    def preload(self, data, pdb_root, topk, num_rbf_node, num_rbf_edge, chain, idx, augment):
        ...
        if chain=='paired':
            ...
            # paired seq
            seqsp, coords, maskp = dict2feature([self._pdb2dict(pdb_path, paired = True, augment = augment)], self.stru_alphabet, self.max_sl)
            ...
            # heavy chain
            seqsh, coords, maskh = dict2feature([self._pdb2dict(pdb_path, chain = 'H', augment = (augment[0], 0))], self.stru_alphabet, self.max_sl)
            _, _, node_featuresh, edge_featuresh = _get_features(seqsh, coords, maskh, topk, num_rbf_node, num_rbf_edge)

            # light chain
            seqsl, coords, maskl = dict2feature([self._pdb2dict(pdb_path, chain = 'L', augment = (0, augment[1]))], self.stru_alphabet, self.max_sl)
            ...
            seq_heavy = data['sequence_heavy']
            seq_light = data['sequence_light']
            if augment[0]:
                seq_heavy = seq_heavy[augment[0]:]
            if augment[1]:
                seq_light = seq_light[:-augment[1]]

            # label_ids here is only used to generate focused mask
            label_ids, label_masks, seq_nums = self.tokenize_stru((seq_heavy, seq_light), max_sl = self.max_sl, add_sep_cls=True)
            ...
            seqsh = self.stru_id2token(seqsh[maskh])
            seqsl = seqsl.squeeze().numpy()
            maskl = maskl.squeeze().numpy().astype(bool)
            seqsl = self.stru_id2token(seqsl[maskl])
            ...
            seqs, coords, mask = dict2feature([self._pdb2dict(pdb_path, chain = 'A', augment = augment)], self.stru_alphabet, self.max_sl)
            ...
            if augment[1]:
                seq_ = seq_[augment[0]: -augment[1]]
            else:
                seq_ = seq_[augment[0]:]
            
            # label_ids here is only used to generate focused mask
            label_ids, label_masks, seq_nums = self.tokenize_stru(seq_, max_sl = self.max_sl, add_sep_cls=True)
            ...
            seqs = self.stru_id2token(seqs[mask])
    ...
    def __getitem__(self, index):
        ...
        bidx=random.randint(0,10)
        augment = (bidx,random.randint(0,10-bidx))
        
        #print('rank {}, idx {}, chain {}'.format(get_rank(),index,chain))

        if chain == 'paired':
            if len(data['sequence_heavy']) + len(data['sequence_light']) + 2 - augment[0] - augment[1] >= self.max_sl: # save one place for cls
                data, chain, idx = self.last_data
        else:
            if len(data['sequence']) + 2 - augment[0] - augment[1] >= self.max_sl: # save one place for cls
                data, chain, idx = self.last_data
        ...
        ret = self.preload(data, self.pdb_root, self.topk, self.num_rbf_node, self.num_rbf_edge, chain, idx, augment)
