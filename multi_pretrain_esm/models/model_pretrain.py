...
class CSSP(nn.Module):
    def __init__(self,
        ...
        self.seq_encoder=SeqEncoder(config, seq_map_dict, seq_encoder)
        ...
        self.seq_projection = nn.Sequential(
            nn.Linear(self.seq_encoder.encoder.embed_dim, self.seq_encoder.encoder.embed_dim),
            nn.ReLU(),
            nn.Linear(self.seq_encoder.encoder.embed_dim, config['embed_dim'])
        )
