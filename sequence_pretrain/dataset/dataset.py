...
class pretrain_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if chain == 'paired':
            ...
            # check X proportion
            elif (data['sequence_heavy'].count('X') + data['sequence_light'].count('X')) / (len(data['sequence_heavy']) + len(data['sequence_light'])) > 0.2:
                data, chain = self.last_data
            ...
            # check X proportion
            elif data['sequence'].count('X') / len(data['sequence']) > 0.2:
                data, chain = self.last_data
