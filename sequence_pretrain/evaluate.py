    ...
    def preload(self, data, chain):
        if chain=='paired':
            ...
            for key in ['cdr3']:
                if len(data[key]):
                    seq_cdr[key] = np.array([self.map_dict[x] for x in data[key]])
                    cdr_len+=len(data[key])
                else:
                    seq_cdr[key] = None
                    
            if cdr_len==0:
                for key in ['cdr1','cdr2']:
                    if len(data[key]):
                        seq_cdr[key] = np.array([self.map_dict[x] for x in data[key]])
                    else:
                        seq_cdr[key] = None
...
def main(args, config):
    ...
    data_loader = create_loader(datasets, samplers, batch_size=[config['batch_size']], num_workers=[10], is_trains=[False], collate_fns=[None])[0]
    ...
    print('load checkpoint from %s'%args.ckpt_path)
    ...
    model.eval()
    with torch.no_grad():
        for i, (input_ids, seq_masks, seq_types, targets) in tqdm(enumerate(metric_logger.log_every(data_loader, print_freq, header))): #tqdm(enumerate(data_loader)):
            ...
            metric_logger.update(acc=(predict_label_id==true_label_id).sum()/len(true_label_id))
            metric_logger.update(loss=loss.cpu().item())
    
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger.global_avg())
    print({k: "{:.3f}".format(meter.global_avg) for k, meter in metric_logger.meters.items()})
