...
class predict_dataset(Dataset):
    def __init__(self, json_files, alphabet, pdb_root, topk, num_rbf_node, num_rbf_edge, max_sl, device, chain_type=None, mlm_probability=0.15):
    ...
    def _process_files(self, json_files):
        if self.chain_type is None:
            ...
            for f in json_files:
                data_=json.load(open(f,'r'))
                data_all+=data_
                type_all+=[self.chain_type]*len(data_)
                idx_all+=[None]*len(data_)
    ...
    def _pdb2dict(self, pdb_path, chain = 'A', paired = False):
        if paired:
            ...
            residues = list(self.parser.get_structure('tp', pdb_path)[0].get_chains())[0].get_list()
        ...
        for res in residues:
            if len(list(res.get_atoms()))<4:
                continue
            atoms_=[]
            for atom in res:
                atoms_.append(atom.id)
            if 'N' not in atoms_:
                continue
...
def main(args, config):
    ...
    datasets = predict_dataset(config['test_file'], alphabet, config['pdb_root'], config['topk'], config['num_rbf_node'], config['num_rbf_edge'], config['max_sl'], device, 'heavy', config['mlm_probability'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/predict_sabdab.yaml')
