...
def train(model, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (node_features, edge_features, labels, label_masks, seq_nums) in tqdm(enumerate(metric_logger.log_every(data_loader, print_freq, header))):
        #print('{} begin time:'.format(i),time.time())
        #btime_=time.time()
        

        #if epoch==0 and i==0:
        #    np.save('node_features.npy',node_features.data.numpy())
        #    np.save('edge_features.npy',edge_features.data.numpy())
        #    np.save('labels.npy',labels.data.numpy())
        #    np.save('label_masks.npy',label_masks.data.numpy())

        optimizer.zero_grad()
        loss = model(node_features, edge_features, labels, label_masks, seq_nums, config['bias_layer_num'])
        loss.backward()
        optimizer.step()
        ...
        #print('{} end time:'.format(i),time.time())
        #print('item time:',time.time()-btime_)
    #print('for end',time.time())
    #print('for time',time.time()-btime_for)
