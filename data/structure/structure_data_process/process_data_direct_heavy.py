...
if 0:
    idx_=0
    seq_npy=np.load('antibody_data/structure_data/{}/seq_split/sequences_{}.npy'.format(chain,idx_),allow_pickle=True)
    for i in tqdm(range(len(seq_npy))):
        pdb_path_='antibody_data/structure_data/{}/split_predict/{}/output_{}_{}.pdb'.format(chain,idx_,idx_,i)
...
data_all=pd.read_csv('antibody_data/merge_sequence/heavy/split_data/unpaired_heavy_rep_sequences_all_0.3_dedup.csv')
...
for idx_ in range(5):
    ...
    seq_npy=np.load('antibody_data/structure_data/{}/seq_split/sequences_{}.npy'.format(chain, idx_),allow_pickle=True)#[begin:]
    ...
    for i in tqdm(range(len(seq_npy))):
        ...
        #print(index_)
        #print('output_{}_{}.pdb'.format(idx_, i))
        try:
    ...
    pd.DataFrame(result_all,columns=['sequence', 'cdr1', 'cdr2', 'cdr3', 'species', 'file_name', 'pdb_file']).to_csv('antibody_data/processed_dataset/struct/{}_3di_seq_{}.csv'.format(chain, idx_), index=False)
