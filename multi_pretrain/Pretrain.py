...
def main(args, config):
    ...
    datasets = [create_dataset(config, alphabet, device)]
    ...
    if args.checkpoint:
        ...
        print('load seq ckpt',config['ckpt_bert_seq'])
        checkpoint = torch.load(config['ckpt_bert_seq'], map_location='cpu')
        state_dict = checkpoint['model']
        missed = model.seq_encoder.load_state_dict(state_dict,strict=False)
        assert len(missed.missing_keys) == 0
        for k in missed.unexpected_keys:
            assert 'cls' in k

        print('load stru ckpt',config['ckpt_bert_stru'])
        checkpoint = torch.load(config['ckpt_bert_stru'], map_location='cpu')
        state_dict = checkpoint['model']
        missed = model.stru_encoder.load_state_dict(state_dict,strict=False)
        assert len(missed.missing_keys) == 0
        for k in missed.unexpected_keys:
            assert 'cls' in k
