...
def _rbf(D, num_rbf, max_, k = 0.1):
    D_min, D_max, D_count = 0., max_, num_rbf
    ...
    D_expand = torch.unsqueeze(D, -1)
        ...
        #print(D_mu.shape)
        #print(D_expand - D_mu)
        #print((k*(D_expand - D_mu) / D_sigma))
        #print(-(k*(D_expand - D_mu) / D_sigma)**2)
    RBF = torch.exp(-(k*(D_expand - D_mu) / D_sigma)**2)
    return RBF

'''
def _rbf(D, num_rbf, max_,print_=True):
    D_min, D_max, D_count = 0., max_, num_rbf
    D_mu = torch.linspace(D_min, D_max, D_count).to(D.device)
    D_mu = D_mu.view([1,1,1,-1])
    D_sigma = (D_max - D_min) / D_count
...
'''
'''
def _get_rbf(A, B, E_idx=None, num_rbf, max_ = 20.0):
    if E_idx is not None:
        D_A_B = torch.sqrt(torch.sum((A[:,:,None,:] - B[:,None,:,:])**2,-1) + 1e-6) #[B, L, L]
        #print('D_A_B edge:',D_A_B.shape)
        #print(D_A_B)
        #np.save('edist_{}.npy'.format(time.time()),D_A_B.numpy())
        D_A_B_neighbors = gather_edges(D_A_B[:,:,:,None], E_idx)[:,:,:,0] #[B,L,K]
        RBF_A_B = _rbf(D_A_B_neighbors, num_rbf, max_ = max_, k = 0.1)
        #RBF_A_B = _rbf(D_A_B_neighbors, 20, max_ = 30.0)
    else:
        D_A_B = torch.sqrt(torch.sum((A[:,:,None,:] - B[:,:,None,:])**2,-1) + 1e-6) #[B, L, 1]
        #print('D_A_B node:',D_A_B.shape)
        #print(D_A_B)
        #np.save('ndist_{}.npy'.format(time.time()),D_A_B.numpy())
        RBF_A_B = _rbf(D_A_B, 8, max_ = 5.0, k = 0.1)
        #RBF_A_B = _rbf(D_A_B, num_rbf, max_ = max_, k = 0.1)
        #RBF_A_B = _rbf(D_A_B, 8, max_ = 5.0)
    return RBF_A_B
'''
def _get_rbf(A, B, E_idx=None, num_rbf=None):
    if E_idx is not None:
        D_A_B = torch.sqrt(torch.sum((A[:,:,None,:] - B[:,None,:,:])**2,-1) + 1e-6) #[B, L, L]
        #print('D_A_B edge:',D_A_B.shape)
        #print(D_A_B)
        #np.save('edist_{}.npy'.format(time.time()),D_A_B.numpy())
        D_A_B_neighbors = gather_edges(D_A_B[:,:,:,None], E_idx)[:,:,:,0] #[B,L,K]
        RBF_A_B = _rbf(D_A_B_neighbors, num_rbf, max_ = 20.0, k = 0.1)
        #RBF_A_B = _rbf(D_A_B_neighbors, 20, max_ = 30.0)
    else:
        D_A_B = torch.sqrt(torch.sum((A[:,:,None,:] - B[:,:,None,:])**2,-1) + 1e-6) #[B, L, 1]
        #print('D_A_B node:',D_A_B.shape)
        #print(D_A_B)
        #np.save('ndist_{}.npy'.format(time.time()),D_A_B.numpy())
        RBF_A_B = _rbf(D_A_B, num_rbf, max_ = 5.0, k = 0.1)
        #RBF_A_B = _rbf(D_A_B, num_rbf, max_ = max_, k = 0.1)
        #RBF_A_B = _rbf(D_A_B, 8, max_ = 5.0)
    return RBF_A_B
