...
def part_state_dict(my_state_dict):
    return {k: my_state_dict[k] for k in my_state_dict if ('lora' in k) or ('embed_types' in k) or ('embed_tokens' in k)}
...
def main(args, config):
    ...
    datasets = [create_dataset(config, alphabet, device, map_dict, mlm_probability = config['mlm_probability'])]
    ...
    param_to_train=[]
    params_all=encoder.named_parameters()
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if utils.is_main_process():
            ...
            save_obj = {
                'model': part_state_dict(model_without_ddp.state_dict()),
